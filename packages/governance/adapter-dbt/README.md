...
This package does not implement dbt artifact loading, project detection,
manifest parsing, normalization logic, dependency mapping, runtime composition,
...
Hosts and runtimes must pass explicit local paths. The adapter contract does
not read implicit shell state and does not assume `process.cwd()` unless that
directory is passed in explicitly as the project directory.
...
- `inputField` for the explicit input member, such as
  `paths.manifestPath` or `options.validationMode`
...
- This package does not detect the dbt project from ambient process state and
  does not invoke dbt commands.
