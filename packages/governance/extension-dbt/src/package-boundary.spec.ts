...
describe('dbt Governance extension package boundary', () => {
  ...
  it('does not import adapter, runtime, or host packages', () => {
    const source = readSourceFiles(path.join(packageRoot, 'src')).join('\n');

    expect(source).not.toMatch(/governance-adapter-dbt/);
    expect(source).not.toMatch(/governance-runtime-dbt/);
    expect(source).not.toMatch(/governance-host-dbt/);
    expect(source).not.toMatch(/packages\/governance\/adapter-dbt/);
